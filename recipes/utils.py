...
def generate_recipe(ingredients: str, cuisine: str, dietary_restrictions: str, difficulty: str) -> dict[str, str]:
    try:
...
def get_nutritional_info(ingredients: list) -> dict[str, str]:
    ...
    query = '\n'.join([f'{i.quantity} {i.name}' for i in ingredients if i.quantity and i.name])
    data = {
        "query": query,
